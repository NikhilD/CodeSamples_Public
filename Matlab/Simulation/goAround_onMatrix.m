function goAround_onMatrix(point, start, dataPoints, colour)
...
prevPt = start;     tau = 2*zVals(row,col);     kerSize = 1;
rowTot = size(zVals,1);   colTot = size(zVals,2);
for i = 1:100
    kern = getMoveKernel(row, col, dataPoints.Hrow, dataPoints.Hcol, rowTot, colTot, kerSize);
    if(isempty(kern.krow))
        return;
    end
    [r,c,value,tau] = getNextPt_wSA(zVals,row,col,kern,tau);
...
function [r,c,value,tau] = getNextPt_wSA(zVals,row,col,kern,tau)
...
if(nnz(deltaZrs)==(lr*lc))
    [r,c,value] = randomMove(kern, row, col, zVals);
    return;
end
[pos,r,c,value] = getDelDir(deltaPos,kern,row,col,zVals,'pos');  
if(isempty(pos))
    [unused,r,c,value] = getDelDir(deltaNeg,kern,row,col,zVals,'neg');
end



function [bitVal, r, c, value] = getDelDir(delMtrx, kern, row, col, zVals, type)
bitVal = []; r = 0;  c = 0; value = 0;  posi = 0;
switch(type)
    case 'pos'
        posi = 1;
...
function [r,c,value] = randomMove(kern, row, col, zVals)
rcVals = getActualRC(kern, row, col);
...
r = rcVals(1,num);  c = rcVals(2,num);  value = zVals(r,c);
