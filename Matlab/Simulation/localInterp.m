function nxPoint = localInterp(motes, neighData, startLocn, obstPoints, rbfunction)
numPoints = length(neighData(1,:));   numObstPts = size(obstPoints,2);
...
dummy = zeros(1,numPoints);     wlRange = motes(1).neigh_radius;
[doExtraElem, plotLocn] = compareLocn(motes, neighData, startLocn);
for m = 1:numPoints
    mt = neighData(1,m);
    [newx, newy] = adjustPosition(neighData(2,m), neighData(3,m), 0);
    X_inp(1,m) = newx;% + startLocn.x; 
    X_inp(2,m) = newy;% + startLocn.y;    
    if(~numObstPts)
...
%     end    
    altiVal(numPoints+1) = 0;%startLocn.Alti;
...
for o = 1:numObstPts
    ...
    gradVal(obstIdx+o) = mean(gradVal(1:obstIdx))/divsr;
    temprVal(obstIdx+o) = mean(temprVal(1:obstIdx))/divsr;
    altiVal(obstIdx+o) = mean(altiVal(1:obstIdx))/divsr;
end
field.x1 = min(X_inp(1,:)); field.x2 = max(X_inp(1,:));
field.y1 = min(X_inp(2,:)); field.y2 = max(X_inp(2,:));

pause(0.25); [pointGPN, dataGPN, trainedgp] = doThinPlate_wRBFGrDe(field, X_inp, gradVal, 1, 8, rbfunction, plotLocn.gpn, wlRange);
% pause(0.25); [pointTEMPR, dataTEMPR, trainedtmp] = doThinPlate_wRBFGrDe(field, X_inp, temprVal, 0, 8, rbfunction, plotLocn.Tempr, wlRange);
% pause(0.25); [pointALTI, dataALTI, trainedalti] = doThinPlate_wRBFGrDe(field, X_inp, altiVal, 0, 8, rbfunction, plotLocn.Alti, wlRange);
...
if((isnan(pointGPN.gradVal))||(isnan(pointTEMPR.gradVal))||(isnan(pointALTI.gradVal))||(~trainedgp)||(~trainedtmp)||(~trainedalti))
    ...
    nxPoint = motes(neighData(1,indx));
    return;
end
plotHist(dataGPN.zVals, 1);
% connectCompo(pointGPN, startLocn, dataGPN, rgb('Brown'),1);
...
% save('data.mat','dataGPN','obstPoints','startLocn');
